...
def bootsODNP(nsamp,nresamp,time,Corrs):
    ...
    plt.plot(time,np.mean(meanCorr,axis=0),label='raw ACF')
    plt.xlabel(r'$time [ps]$')
    plt.ylabel(r'$C(t)$')
    
    plt.plot(time,np.mean(meanCorrFit,axis=0),label='exponential fit')
    plt.xlabel(r'$time [ps]$')
    plt.ylabel(r'$C(t)$')
    plt.savefig('corr.png')
    plt.close()

    frequency = np.linspace(-4,4,100)
    plt.plot(frequency,spectral(frequency,popt[0],popt[1],popt[2],popt[3],popt[4]))
