...
def main(args):    
    ...
    Tnew = []
    names = glob.glob('k_sigma'+system+'*.txt')    
    for name in names:
        pattern = '(\d{3})\D'
    ...
    ax2.set_ylabel(r'$k_{\sigma}$',fontsize=14)

    fig.savefig('k_'+prefix+'_'+system+'.png',dpi=1000)
    fig.savefig('k_'+prefix+'_'+system+'.eps',format='eps')
    plt.show()
