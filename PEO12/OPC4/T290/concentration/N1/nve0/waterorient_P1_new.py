...
def P1(topFile,trajFile):
    ...
    for i,chunk in enumerate(md.iterload(trajFile,top=topFile,skip=timeOrigin,
      ...
      if i!=0:
        ...
        for t,frame in enumerate(chunk[1:]):
          ...
          #dot_u = np.einsum('ij,ij->i',u,init_u)
          thisCorr[t] = np.einsum('ij,ij->',u,init_u)
          
          #P1 = dot_u
          #thisCorr[t] = np.sum(P1)
          
          #print(timeit.time()-start,t)
        print(timeit.time()-initial)
        Corr += thisCorr
        nChunks += 1
        tempCorr = Corr/float(nChunks)
        np.savetxt('OrientCorr_P1.txt',(timevals,tempCorr))
        plt.plot(timevals,tempCorr)
        plt.xlabel('time (ps)')
        plt.ylabel('Orientational ACF')
        plt.savefig('OrientCorr_P1.png')
        plt.close()
  ...
  np.savetxt('OrientCorr_P1.txt',(timevals,Corr))
  np.savetxt('OrientCorrfit_P1.txt',(timevals,Corr_fit))
  ...
  plt.ylabel('Orientational ACF')
  plt.legend()
  plt.savefig('OrientCorrfit_P1.png')
