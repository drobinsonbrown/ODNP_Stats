...
def bootsODNP(nsamp,nresamp,time,Corrs):
    index = 0
    freqs = np.logspace(-9,-1)
    meanJ = np.zeros((nresamp,len(freqs)))
    ...
    while index < nresamp:
        ...
        meanJ[index,:] = spectral(freqs,popt[0],popt[1],popt[2],popt[3],popt[4])
    ...
    return meanCorr, meanJ, k_sigma, xi
...
def main(args):
    ...
    [meanCorr,meanJ,k_sigma,xi]=bootsODNP(nsamp,nresamp,time,Corrs)

    # save a representation of average ACF
    Corr=np.mean(meanCorr,axis=0)
    J = np.mean(meanJ,axis=0)
    np.savetxt('avg'+prefix+'.txt',(time,Corr))
    np.savetxt('avgJ'+'.txt',(freqs,J))
    plt.plot(freqs,J)
    plt.show()
    plt.close()
