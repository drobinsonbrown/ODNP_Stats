...
def timeCorrFunc(trajchunk, radNames='@Or,Nr', nucNames='@HW'):
  ...
  cutoff=float(0.8)
  OrInd = trajchunk.top.select('@Or')
  radInds = trajchunk.top.select(radNames)
  nucInds = trajchunk.top.select(nucNames)
  OInds = pt.search_neighbors(trajchunk,'(@Or<:8.0)&@O')[0]
  nucInds = []
  for i,ind in enumerate(OInds):
    nucInds.append(ind+1)
    nucInds.append(ind+2)
  #nucInds = md.compute_neighbors(trajchunk,cutoff,OrInd,haystack_indices=nucInds)[0]
...
def compute(topFile,trajFile):
  ...
  chunkSpace = 300 #10 ps should be spaced far enough apart for uncorrelated configurations
